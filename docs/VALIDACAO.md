...
**Resultado local: 8 testes, zero falhas, zero erros e zero testes ignorados; BUILD SUCCESS.**

| Critério | Evidência |
| --- | --- |
| Compilação e empacotamento | `mvn clean verify`, código 0, JAR `target/app.jar` gerado |
| Estoque e auditoria | 8 testes com banco real, incluindo rollback, concorrência, saldo insuficiente e overflow |
| CLI executável | `java -jar target/app.jar demo`, encerrado com código 0 |
| Persistência entre processos | Cadastro, entrada, saída e leitura em JVMs separadas mantiveram saldo 2 e dois movimentos |
| Códigos de erro | Saída insuficiente: 2; SKU duplicado: 3 |
